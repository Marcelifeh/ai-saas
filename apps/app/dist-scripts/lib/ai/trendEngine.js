...
exports.getSerpApiTrends = getSerpApiTrends;
exports.getRedditTrends = getRedditTrends;
exports.getHackerNewsTrends = getHackerNewsTrends;
exports.collectTrendSignals = collectTrendSignals;
exports.generateNiches = generateNiches;
exports.clusterNiches = clusterNiches;
exports.scoreNiche = scoreNiche;
...
function buildSourceResult(source, status, data, confidence, details) {
    return {
        source,
        tier: signalReliabilityService_1.RELIABLE_SIGNAL_SOURCE_TIERS[source],
        status,
        data,
        confidence,
...
function appendDetails(primary, secondary) {
    return secondary ? `${primary} ${secondary}` : primary;
}
function computeSignalAgreement(source, peers) {
...
function rebalanceSourceConfidence(sources) {
...
function normalizeSourceWeights(sources) {
    ...
    return sources.map((source) => ({
        ...source,
        normalizedWeight: totalConfidence > 0 && source.data.length > 0 && source.confidence > 0
            ? roundConfidence(source.confidence / totalConfidence)
            : 0,
    }));
}
function calculateSignalConfidence(sources) {
...
function getCachedGoogleTrendsFallback(details, status) {
    const cached = cache_1.globalCache.get(GOOGLE_TRENDS_CACHE_KEY);
...
async function getPersistentSourceFallback(source, snapshotKey, details, status) {
    const snapshot = await (0, signalReliabilityService_1.getPersistentSignalSnapshot)(source, snapshotKey, details);
    if (!snapshot) {
        return null;
    }
    return {
        ...
        details: appendDetails(details, `Using persisted snapshot from ${snapshot.ageMinutes} minute${snapshot.ageMinutes === 1 ? "" : "s"} ago.`),
    };
}
function extractSerpApiQueries(payload) {
    if (!payload || typeof payload !== "object")
        return [];
    const p = payload;
    // Format A (classic): trending_searches[].query
    if (Array.isArray(p.trending_searches) && p.trending_searches.length > 0) {
        const queries = p.trending_searches
    ...
    if (Array.isArray(p.trending_now_shows_searches) && p.trending_now_shows_searches.length > 0) {
        const queries = [];
        for (const item of p.trending_now_shows_searches) {
    ...
    if (Array.isArray(p.realtime_trending_searches) && p.realtime_trending_searches.length > 0) {
        const queries = p.realtime_trending_searches
            .map((item) => (item?.title ?? item?.query))
            .filter((q) => typeof q === "string" && q.trim().length > 0)
            .slice(0, 20);
        if (queries.length > 0)
            return queries;
...
async function fetchSerpApiTrendingQueries(apiKey) {
...
async function getGoogleProxyFallback(details) {
    ...
    const liveResult = buildSourceResult("google_trends", "ok", proxyResponse.queries, 0.94, `${details} Recovered through the SerpAPI Google Trends proxy.`);
    cache_1.globalCache.set(GOOGLE_TRENDS_CACHE_KEY, liveResult, GOOGLE_TRENDS_CACHE_TTL_MS);
    await (0, signalReliabilityService_1.persistSignalSnapshot)({
    ...
    await (0, signalReliabilityService_1.recordSourceSuccess)("google_trends");
    return liveResult;
}
/**************************************************************
 * UTILITIES
...
async function getGoogleTrends() {
    const health = await (0, signalReliabilityService_1.getSourceHealth)("google_trends");
    if ((0, signalReliabilityService_1.isSourceInCooldown)(health)) {
        const cooldownFallback = await getPersistentSourceFallback("google_trends", GOOGLE_TRENDS_SNAPSHOT_KEY, `Google Trends is in cooldown until ${health.cooldownUntil}.`, "cooldown");
        if (cooldownFallback) {
    ...
    try {
        ...
        if (!rssResponse.ok) {
            const status = rssResponse.status === 429 ? "rate_limited" : "blocked";
            ...
            await (0, signalReliabilityService_1.recordSourceFailure)("google_trends", details);
        ...
        const status = message.includes("429") ? "rate_limited" : "error";
        ...
        await (0, signalReliabilityService_1.recordSourceFailure)("google_trends", message);
        const persistentFallback = await getPersistentSourceFallback("google_trends", GOOGLE_TRENDS_SNAPSHOT_KEY, message, status);
        if (persistentFallback) {
            return persistentFallback;
        }
...
async function getSerpApiTrends() {
    ...
    const health = await (0, signalReliabilityService_1.getSourceHealth)("serpapi_trends");
    if ((0, signalReliabilityService_1.isSourceInCooldown)(health)) {
        const cooldownFallback = await getPersistentSourceFallback("serpapi_trends", SERPAPI_TRENDS_SNAPSHOT_KEY, `SerpAPI Trends is in cooldown until ${health.cooldownUntil}.`, "cooldown");
        if (cooldownFallback) {
            return cooldownFallback;
        }
    ...
    try {
        ...
        if (!response.ok) {
            await (0, signalReliabilityService_1.recordSourceFailure)("serpapi_trends", response.details);
            const fallback = await getPersistentSourceFallback("serpapi_trends", SERPAPI_TRENDS_SNAPSHOT_KEY, response.details, response.status);
            if (fallback) {
                return fallback;
            }
        ...
        await (0, signalReliabilityService_1.recordSourceSuccess)("serpapi_trends");
        return liveResult;
    }
    catch (err) {
        const details = err instanceof Error ? err.message : "Unknown SerpAPI error";
        await (0, signalReliabilityService_1.recordSourceFailure)("serpapi_trends", details);
        const fallback = await getPersistentSourceFallback("serpapi_trends", SERPAPI_TRENDS_SNAPSHOT_KEY, details, "error");
        if (fallback) {
            return fallback;
        }
...
async function getRedditTrends() {
    const health = await (0, signalReliabilityService_1.getSourceHealth)("reddit");
    if ((0, signalReliabilityService_1.isSourceInCooldown)(health)) {
        const cooldownFallback = await getPersistentSourceFallback("reddit", REDDIT_SNAPSHOT_KEY, `Reddit source is in cooldown until ${health.cooldownUntil}.`, "cooldown");
        if (cooldownFallback) {
            return cooldownFallback;
        }
    ...
    const status = titles.length > 0 ? "ok" : "error";
    ...
    if (titles.length > 0) {
        ...
        await (0, signalReliabilityService_1.recordSourceSuccess)("reddit");
        return liveResult;
    }
    await (0, signalReliabilityService_1.recordSourceFailure)("reddit", details ?? "Reddit returned no usable titles.");
    const fallback = await getPersistentSourceFallback("reddit", REDDIT_SNAPSHOT_KEY, details ?? "Reddit returned no usable titles.", "error");
    if (fallback) {
        return fallback;
    }
...
async function getHackerNewsTrends() {
    const health = await (0, signalReliabilityService_1.getSourceHealth)("hacker_news");
    if ((0, signalReliabilityService_1.isSourceInCooldown)(health)) {
        const cooldownFallback = await getPersistentSourceFallback("hacker_news", HACKER_NEWS_SNAPSHOT_KEY, `Hacker News is in cooldown until ${health.cooldownUntil}.`, "cooldown");
        if (cooldownFallback) {
            return cooldownFallback;
        }
    ...
    try {
        ...
        if (!idsResponse.ok) {
            ...
            await (0, signalReliabilityService_1.recordSourceFailure)("hacker_news", details);
        ...
        const stories = await Promise.allSettled(topIds.map(async (id) => {
        ...
        const titles = stories
            ...
            .filter((story) => Boolean(story && typeof story.title === "string" && story.title.trim().length > 0))
        ...
        await (0, signalReliabilityService_1.recordSourceSuccess)("hacker_news");
        return liveResult;
    }
    catch (err) {
...
async function generateNiches(signals) {
    ...
    const dynamicContext = (0, trendContext_1.getDynamicContext)();
    const prompt = `
...
`;
    const completion = await (0, aiGateway_1.chatCompletionSafe)({
        model: "gpt-4o-mini",
        temperature: 0.85,
...
async function embedTexts(texts) {
    if (!texts || texts.length === 0)
        return [];
    const response = await (0, aiGateway_1.createEmbeddingsSafe)({
        input: texts,
...
function estimateDemand(niche) {
    ...
    demandKeywords.forEach((kw) => {
        if (niche.toLowerCase().includes(kw))
            score += 5;
    });
...
`;
    ...
    try {
        ...
        if (Array.isArray(arr) && arr.length === niches.length)
            return arr;
        return fallback;
    }
    catch {
...
async function discoverTrends() {
    const cached = await cache_1.globalCache.getAsync('global_trends_v2');
    if (cached) {
    ...
    cache_1.globalCache.set('global_trends_v2', finalData, 30 * 60 * 1000); // Cache for 30 minutes
    return finalData;
}
