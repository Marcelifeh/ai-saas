...
exports.buildFromPatterns = buildFromPatterns;
exports.expandPatternFamilies = expandPatternFamilies;
exports.mutatePatternFamily = mutatePatternFamily;
exports.enhanceSlogans = enhanceSlogans;
exports.runEliteSloganEngine = runEliteSloganEngine;
exports.generateHighPotentialSlogans = generateHighPotentialSlogans;
exports.generateDynamicSlogans = generateDynamicSlogans;
exports.extractPersonas = extractPersonas;
exports.extractEmotions = extractEmotions;
exports.generateVariants = generateVariants;
exports.normalizeImagePrompts = normalizeImagePrompts;
exports.learnPattern = learnPattern;
exports.getPatternScore = getPatternScore;
exports.predictVirality = predictVirality;
exports.getTrendScore = getTrendScore;
exports.syncMarketplace = syncMarketplace;
// Get top and low-performing patterns for a niche from DB
async function getTopPatterns(niche) {
    try {
...
async function getLowPerformingPatterns(niche) {
    try {
        const patterns = await prisma_1.prisma.sloganPattern.findMany({
...
function injectWinningBias(slogan, patterns) {
    let boost = 0;
...
function detectEmotionCluster(slogan) {
    if (/proud|built|born|real|true|pure|unapologetic|unbothered|certified|official|dedicated/i.test(slogan))
        return "PRIDE";
    if (/lol|funny|nah|sarcasm|humor|not people|leave me alone/i.test(slogan))
        return "HUMOR";
    if (/don’t|never|against|no |not |rebellion|anti|feral|chaos|wild/i.test(slogan))
        return "REBELLION";
    if (/club|tribe|crew|squad|team|family|belong|together/i.test(slogan))
        return "BELONGING";
    return "GENERIC";
}
// Enrich trend keywords into language style
function enrichTrendLanguage(trends) {
...
function normalizeDynamic(values) {
...
function stretchScore(score, exponent = 0.85) {
...
function applyBatchNormalization(items) {
    ...
    return items.map((it, idx) => {
        const normalizedScore = nh[idx] * weights.hook +
...
function extractPatternFeatures(slogan, niche) {
...
function trainWeights(data) {
    const weights = {};
    for (const item of data) {
        const score = computePerformanceScore(item);
...
async function loadPatternWeights(niche) {
    try {
    ...
    try {
        const prows = await prisma_1.prisma.sloganPattern.findMany({ where: { niche: niche?.trim().toLowerCase() || undefined }, take: 200 });
        if (prows && prows.length > 0) {
            // Map pattern success into simple features: presence of pattern words
            const pseudo = prows.map((p) => ({
                slogan: p.pattern,
...
function applyLearningBoost(slogan, baseScore, weights, niche) {
...
function applyExploration(score, explorationEpsilon = 0.1) {
...
async function getTopPerformingSlogans(niche, limit = 5) {
    ...
    try {
        const patterns = await prisma_1.prisma.sloganPattern.findMany({ where: { niche: niche?.trim().toLowerCase() }, orderBy: { score: 'desc' }, take: limit });
        if (patterns && patterns.length > 0)
            return patterns.map((p) => p.pattern).slice(0, limit);
    }
    catch (_) { /* ignore */ }
    return [];
}
 ...
 */
function extractBehaviorSignals(niche) {
    const map = {
        pickleball: [
...
function isHumanPhrase(s) {
    ...
    if (trimmed.split(/[,.;]/).length > 2 && trimmed.split(/\s+/).length > 8)
        return false;
    return true;
}
/**
 ...
 */
function computeConversationalScore(s) {
    if (!s)
        return 0;
    const lower = s.toLowerCase();
    let score = 0;
    if (/\bjust one more\b/.test(lower))
        score += 10;
    if (/\bsorry\b|\bmy bad\b|\bomg\b|\bwhoops\b/.test(lower))
        score += 8;
    if (/\bhappens\b|\bdink\b|\bserve\b|\bkitchen\b/.test(lower))
        score += 6;
    if (/[!?]/.test(s))
        score += 4;
...
function normalizeSalesSignals(value) {
    if (!value || typeof value !== "object")
        return {};
    return value;
}
function extractMeaningfulTokens(value) {
    return dedupeStrings(sanitizePhrase(value)
        .split(/\s+/)
        .filter((token) => token.length > 2));
}
function buildCompounds(tokens) {
    const compounds = [];
...
function pickOne(values, seed, fallback) {
    if (values.length === 0)
        return fallback;
    return pickByHash(values, 1, seed)[0] ?? fallback;
}
function buildPersonaLexicon(personaKey, mode) {
    const broad = {
...
function pickByHash(arr, count, seed) {
    const result = [];
    const used = new Set();
    let s = seed;
...
function buildNicheLexicon(input, persona, mode) {
...
function deriveNicheIdentities(niche, persona) {
...
function deriveNicheEmotions(niche, persona) {
    const profile = PERSONAS.find((candidate) => candidate.key === persona) ?? PERSONAS[PERSONAS.length - 1];
    return buildNicheLexicon({ niche }, profile, "safe").emotionWords;
}
function detectWinningClusters(niche, audience, lexicon, signals, persona) {
    const haystack = [
        niche,
        audience || "",
        persona.voice,
...
function clusterLeadKeyword(cluster, fallback) {
    if (!cluster)
        return fallback;
    return cluster.keywords[0] ?? fallback;
}
function selectClusterAngles(clusters, lexicon, context) {
...
function buildFragmentBuckets(lexicon) {
    ...
    const audience = dedupeStrings(lexicon.audienceTerms.flatMap((term, index) => {
...
function renderHumorObject(scene, activity, identity) {
    const lowerScene = normalizeKeyword(scene);
    if (lowerScene.includes("stars") || lowerScene.includes("fire"))
        return "campfires";
    if (lowerScene.includes("off the grid"))
        return "off-grid life";
    if (lowerScene.includes("game night"))
        return "game night";
    if (lowerScene.includes("couch"))
        return "the couch";
    if (lowerScene.includes("at home"))
        return "staying home";
    const lowerActivity = normalizeKeyword(activity);
    if (lowerActivity.includes("walking the dog"))
        return "dog walks";
    if (lowerActivity.includes("rolling dice"))
        return "rolling dice";
    if (lowerActivity.includes("playing to win"))
        return "winning";
    if (lowerActivity.includes("sleeping outside"))
        return "sleeping outside";
    if (lowerActivity.includes("camping"))
        return "camping";
    return toLowerPhrase(identity);
}
function normalizeContrastTarget(target) {
    const lower = toLowerPhrase(target);
    if (lower === "other plans")
        return "plans";
    if (lower === "group chats")
        return "group chats";
    if (lower === "city noise")
        return "city noise";
    if (lower === "small talk")
...
function buildAttitudeCandidates(lexicon, context, clusters) {
...
function buildHumorCandidates(lexicon, context, clusters) {
    ...
    const isIntrovertNiche = /introvert|sarcasm|social|cozy|nap|coffee|quiet|chaos/.test(`${clusters.humorAngle} ${identity.toLowerCase()}`);
...
function buildIdentityCandidates(lexicon, context, clusters) {
...
function buildContrastCandidates(lexicon, context, clusters) {
...
function buildStatementCandidates(lexicon, context, clusters) {
...
function detectPatternFamily(slogan) {
    const lower = slogan.toLowerCase();
    if (/^too\s+.+\s+to\s+|^too\s+.+\s+for\s+/.test(lower) || /^no\s+.+\.\s+no problem/.test(lower))
        return "ATTITUDE";
    if (/\bhere for\b|\bcame for\b|\bleave me alone\b|\bnot conversation\b|\bnot people\b/.test(lower))
        return "HUMOR";
    if (/>|\bless\s+.+\s+more\s+.+\b|\boutside\.\s+peace inside\b/.test(lower))
        return "CONTRAST";
    if (/\bstate of mind\b|\blooks good on me\b|\bon purpose\b|\bwhole personality\b/.test(lower))
        return "IDENTITY";
    return "STATEMENT";
}
function structureSignature(slogan) {
    const lower = slogan.toLowerCase().trim();
    if (/^too\s+.+\s+to\s+.+/.test(lower))
        return "too_x_to_y";
    if (/^too\s+.+\s+for\s+.+/.test(lower))
        return "too_x_for_y";
    if (/^here for\s+.+,\s+not\s+.+/.test(lower))
        return "here_for_x_not_y";
    if (/^came for\s+.+,\s+not\s+.+/.test(lower))
        return "came_for_x_not_y";
    if (/^no\s+.+\.\s+no problem\.?$/.test(lower))
        return "no_x_no_problem";
    if (/^.+\s+>\s+.+$/.test(lower))
        return "x_gt_y";
    if (/^leave me alone\./.test(lower))
        return "leave_me_alone";
    if (/^.+\.\s+zero\s+.+$/.test(lower))
        return "x_zero_y";
    if (/^wild outside\.\s+peace inside\.?$/.test(lower))
        return "outside_inside";
    if (/^.+\s+state of mind\.?$/.test(lower))
 ...
 */
function patternFingerprint(slogan) {
    return slogan
        .toLowerCase()
        .replace(/[^\w\s]/g, "")
 ...
 */
function computeNicheSpecificity(slogan, niche) {
    const lower = slogan.toLowerCase();
 ...
 */
function computeNoveltyBonus(slogan, usedFingerprints) {
 ...
 */
function computeEmotionalIntensity(slogan) {
    const lower = slogan.toLowerCase();
    let score = 0;
    if (/\b(obsessed|devoted|relentless|unstoppable|fearless|unapologetic|fierce|raw|fire|wild)\b/.test(lower))
        score += 12;
    if (/\b(love|hate|proud|real|soul|born|true|heart|broke|alive|free|rise)\b/.test(lower))
        score += 8;
    if (/[!]/.test(slogan))
        score += 4;
    return Math.min(score, 20);
}
function computeClusterAlignment(slogan, clusters) {
    const lower = slogan.toLowerCase();
    let score = 0;
    for (const cluster of clusters.slice(0, 3)) {
        const matched = cluster.keywords.some((keyword) => lower.includes(keyword.toLowerCase()));
        if (matched)
            score += cluster.weight * 8;
    }
    return clamp(Math.round(score), 0, 28);
}
function computeViralReadiness(slogan) {
    const lower = slogan.toLowerCase();
    let score = 0;
    if (slogan.length <= 40)
        score += 8;
    if (/\bnot\b|\bdon't\b|\bno\b|>/.test(lower))
        score += 9;
    if (/[!?]/.test(slogan))
        score += 4;
    if (/\bi\b|\bme\b|\bmy\b|\bi'm\b/.test(lower))
        score += 7;
    if (slogan.split(/\s+/).length <= 7)
        score += 7;
    return clamp(score, 0, 25);
}
/**
...
function scoreCandidate(text, clusters) {
    const cleaned = cleanupPhrase(text);
    const wordCount = cleaned.split(/\s+/).length;
    let score = computeViralReadiness(cleaned) + computeClusterAlignment(cleaned, clusters);
    if (wordCount <= 6)
        score += 6;
    if (/\bnot people\b|\bno problem\b|\bzero\b|>/.test(cleaned.toLowerCase()))
        score += 4;
    if (/\btoo\s+.+\s+to\s+care\b/.test(cleaned.toLowerCase()))
        score -= 4;
    return score;
}
function enforcePatternDiversity(candidates, lexicon, clusters) {
    ...
    for (const candidate of sorted) {
        ...
        if (!cleaned || isAwkwardPhrase(cleaned, lexicon))
            continue;
        ...
        if (usedSignatures.has(signature) || familyCount >= familyLimit)
            continue;
...
function buildPatternFamilyCandidates(lexicon, context, clusters) {
...
function deriveActivityFromNiche(niche) {
    const lower = niche.toLowerCase();
    if (/camp|outdoor|hike|rv|trail|forest|summer camp/.test(lower))
        return "camping";
    if (/game|gaming|board|dice|dnd|tabletop/.test(lower))
        return "playing";
    if (/fitness|gym|lift|run|workout|train/.test(lower))
        return "training";
    if (/faith|church|jesus|god|christian|bible/.test(lower))
        return "walking in faith";
    if (/fashion|style|outfit|streetwear/.test(lower))
        return "getting dressed";
    if (/pet|dog|cat|paw|rescue/.test(lower))
        return "spoiling pets";
    if (/cook|chef|culinary|recipe|kitchen|bak|pastry|brunch|foodie/.test(lower))
        return "cooking";
    if (/coffee|espresso|latte|barista|brew|caffeine/.test(lower))
        return "brewing the perfect cup";
    if (/photo|camera|lens|darkroom|film photo|photographer/.test(lower))
        return "shooting";
    if (/paint|artist|illustrat|sketch|draw|gallery|studio/.test(lower))
        return "creating";
    if (/yoga|meditat|mindful|wellness|zen|pilates/.test(lower))
        return "centering";
    if (/garden|plant|botanical|herb|bloom|flower/.test(lower))
        return "tending the garden";
    if (/book|read|library|librarian|bookworm|bibliophile/.test(lower))
        return "reading";
    if (/teach|educator|classroom|professor|tutor/.test(lower))
        return "teaching";
    if (/nurs|doctor|healthcare|hospital|scrubs|medical/.test(lower))
        return "showing up for people";
    if (/music|guitar|drum|band|vinyl|record|musician/.test(lower))
        return "playing";
    if (/travel|wander|backpack|nomad|van life|road trip|passport/.test(lower))
        return "exploring";
    if (/wine|beer|brewery|winery|craft beer/.test(lower))
        return "sipping slowly";
    if (/marathon|cycling|triathlon|trail run/.test(lower))
        return "chasing the finish line";
    return "doing my thing";
}
function buildNicheContext(niche, audience, persona, lexicon) {
...
function buildNarrativeSlogans(lexicon, context, mode, clusters) {
...
function isAwkwardPhrase(value, lexicon) {
    ...
    if ((cleaned.match(/[/:.]/g) || []).length > 2)
        return true;
    if (/^[A-Z\s]+$/.test(cleaned) && tokens.length > 2)
        return true;
    if (tokens.every((token) => token.length <= 3 || NOISE_WORDS.has(token)))
        return true;
    if (tokens.some((token) => NOISE_WORDS.has(token)) && !lexicon.anchors.some((anchor) => cleaned.toLowerCase().includes(anchor)))
        return true;
    if (tokens.some((token, index) => index > 0 && token === tokens[index - 1]))
        return true;
    const clauses = cleaned.split(/[/:.]/).map((part) => part.trim()).filter(Boolean);
    if (clauses.length >= 2 && clauses.every((clause) => clause.split(/\s+/).length === 1))
        return true;
    if (/^\w+\.\s*\w+\.?$/i.test(cleaned) && clauses.length === 2)
        return true;
    if (/\b(love|cozy|sleepy|unbothered|faithful|proud)\.\s*\b/i.test(cleaned))
        return true;
    return false;
}
function humanizeSloganCandidate(value, lexicon) {
    const cleaned = cleanupPhrase(value)
        .split(/[/:]/)
        .map((part, index) => {
...
function ensureAnchorPresence(slogan, lexicon, seed) {
    const lower = slogan.toLowerCase();
    if (lexicon.anchors.some((anchor) => lower.includes(anchor)))
        return slogan;
    const anchor = pickOne(lexicon.anchors, seed, lexicon.anchors[0] ?? "identity");
    return cleanupPhrase(`${toTitleCase(anchor)}. ${slogan}`);
}
function buildDynamicSlogan(lexicon, buckets, mode, index) {
    const primaryFamilies = ["identity", "emotion", "motion"];
    const secondaryFamilies = ["emotion", "motion", "emphasis", "signal", "audience", "identity"];
    const primaryFamily = pickOne(primaryFamilies.filter((family) => buckets[family]?.length > 0), lexicon.seed + index * 97, "identity");
    const includeSecondary = ((lexicon.seed + index) % (mode === "edgy" ? 2 : 3)) !== 0;
    const chosenFamilies = [primaryFamily];
    if (includeSecondary) {
        const secondaryFamily = pickOne(secondaryFamilies.filter((family) => family !== primaryFamily && buckets[family]?.length > 0), lexicon.seed + index * 131, primaryFamily);
        if (secondaryFamily !== primaryFamily)
            chosenFamilies.push(secondaryFamily);
    }
    const parts = chosenFamilies.map((family, familyIndex) => pickOne(buckets[family], lexicon.seed + index * 149 + familyIndex * 17, lexicon.anchors[0] ?? "identity"));
    ...
    const renderedParts = parts.map((part, partIndex) => {
        if (transform === 0 && partIndex === 0)
            return toTitleCase(part);
        if (transform === 1 && partIndex === parts.length - 1 && part.split(/\s+/).length <= 2)
            return toTitleCase(part);
        if (transform === 2 && partIndex === 0 && part.split(/\s+/).length <= 3)
            return toTitleCase(part);
        return part;
    });
...
function buildNicheSlogans(niche, identities, emotions, mode, audience, salesSignals = {}) {
    ...
    const dynamicCandidates = enforcePatternDiversity(generated.map((value) => ({
        text: humanizeSloganCandidate(value, mergedLexicon),
        family: detectPatternFamily(value),
    })), mergedLexicon, clusters).map((candidate) => candidate.text);
...
// ─── Scoring Functions ────────────────────────────────────────────────────────
function computeWearability(slogan) {
...
function computeMemorability(slogan) {
...
function computeIdentityScore(slogan) {
    const lower = slogan.toLowerCase();
    let score = 0;
    if (/\b(i am|i'm|we are|we're|my|our)\b/.test(lower))
        score += 25;
    if (/\b(life|soul|born|real|true|pure|proud|forever|always)\b/.test(lower))
        score += 20;
    if (/\b(unapologetic|unbothered|certified|official|dedicated)\b/.test(lower))
        score += 15;
    // Penalize "I love X" or "I like X" statements
    if (isStatement(slogan))
        score -= 30;
...
function computeRecognitionScore(s, niche) {
    ...
    if (lower.includes(lowerNiche))
        score += 12;
    // High-Conversion Behavioral Hooks
    if (/\b(again|just one more|can't stop|another one|retry|respawn)\b/i.test(lower))
        score += 10;
    if (/\b(you know|you get it|insider|if you know)\b/i.test(lower))
        score += 8;
    if (/\b(mode|energy|inside|built)\b/i.test(lower))
        score += 6;
...
function isOverusedWord(word, niche, batchStats) {
    const usage = batchStats[word.toLowerCase()] || 0;
    // Dynamic suppression if word appears in > 30% of candidates
    return usage > 0.3;
}
function computeEmotionScore(slogan) {
    const lower = slogan.toLowerCase();
    let score = 0;
    if (/\b(fierce|unstoppable|fearless|relentless|dominant|clutch)\b/.test(lower))
        score += 30;
    if (/\b(proud|bold|devoted|driven|focused|strong|powerful)\b/.test(lower))
        score += 20;
    if (/\b(blessed|faithful|called|wild|free|alive|raw)\b/.test(lower))
        score += 15;
    if (/\b(love|joy|hope|peace|grace|brave|heart)\b/.test(lower))
        score += 10;
    return clamp(score + 30, 0, 100);
}
function computeHookScore(s, niche) {
    let score = 0;
    const lower = s.toLowerCase();
    if (/[^\w\s]/.test(s))
        score += 5; // symbols/emphasis (!, ?, >)
    if (/\b(not|but|instead|over|vs)\b/i.test(lower))
        score += 8; // contrast
    // Add Recognition Score to Hook weight
    score += computeRecognitionScore(s, niche);
    if (s.length < 50)
        score += 5; // wearable length
    // Original hook rules as backup
    if (/^(built|born|real|not|you|they|we|just|only|never|always|this|that)/.test(lower))
        score += 5;
    if (/\b(different|hits|calling|mode|season)\b/.test(lower))
        score += 5;
    return clamp(score, 0, 100);
}
function computeClevernessScore(s) {
    let score = 0;
    const lower = s.toLowerCase();
    if (/(pro|certified|expert|official|elite|specialist)\b/i.test(lower))
        score += 6;
    if (/(powered by|fueled by|driven by|brought to you by)\b/i.test(lower))
        score += 6;
    if (/(it's not|you wouldn't|trust me|the secret|you missed)\b/i.test(lower))
        score += 8;
    if (/\b(survivor|addict|mode|vibe)\b/i.test(lower))
        score += 4;
    return clamp(score, 0, 100);
}
function computePunchScore(slogan) {
    let score = 50;
    if (/[.!]$/.test(slogan.trim()))
        score += 15;
    if (/\w+\.\s*\w+/.test(slogan))
        score += 10;
    if (slogan.split(/\s+/).length <= 5)
        score += 15;
    if (/[A-Z]{2,}/.test(slogan))
        score += 10;
    return clamp(score, 0, 100);
}
function computeVisualFit(slogan) {
...
function computeSymmetry(slogan) {
    const parts = slogan.split(/[.,!?;:|—]/).map((p) => p.trim()).filter(Boolean);
    if (parts.length === 2 && Math.abs(parts[0].length - parts[1].length) <= 8)
        return 80;
    if (parts.length === 3)
        return 75;
    return 55;
}
function computeLineBreakPotential(slogan) {
    if (/[.\n|—]/.test(slogan))
        return 80;
    if (slogan.split(/\s+/).length <= 5)
        return 70;
...
function computeFontImpact(slogan) {
...
function computeContrastScore(slogan) {
    const lower = slogan.toLowerCase();
    if (/not |never |no |without |despite /.test(lower))
        return 80;
    if (/ but | yet | though /.test(lower))
        return 75;
    return 50;
}
function computeCuriosityGap(slogan) {
    const lower = slogan.toLowerCase();
    let score = 40;
    if (/\?/.test(slogan))
        score += 25;
    if (/\b(secret|hidden|truth|real|only)\b/.test(lower))
        score += 20;
    if (/\b(different|hits|deeper|more)\b/.test(lower))
        score += 15;
    return clamp(score, 0, 100);
}
function computeEmotionalTrigger(slogan) {
    const lower = slogan.toLowerCase();
    let score = 30;
    if (/\b(love|hate|fear|joy|pride|pain|hope|anger|peace)\b/.test(lower))
        score += 30;
    if (/\b(heart|soul|spirit|fire|blood|tears|smile)\b/.test(lower))
        score += 20;
    if (/[!]/.test(slogan))
        score += 10;
    return clamp(score, 0, 100);
...
function isGeneric(s) {
    ...
    if (wordCount <= 3 && !/[^\w\s]/.test(s))
        return true;
    return genericPatterns.some(p => p.test(s));
}
function detectEmotionalClustersForNiche(niche) {
    const map = {
...
function computeGenericPenalty(slogan) {
...
function computeMarketSignalScore(signals, slogan) {
    if (!signals || Object.keys(signals).length === 0)
        return 0;
    let score = 0;
    if (signals.confidence)
        score += clamp(signals.confidence * 50, 0, 50);
    const keywords = signals.topKeywords || [];
    const lower = slogan.toLowerCase();
    for (const kw of keywords) {
        if (lower.includes(normalizeKeyword(kw)))
            score += 8;
    }
    return clamp(score, 0, 100);
}
function computeNaturalPhraseBonus(slogan) {
    const lower = slogan.toLowerCase();
    let score = 0;
    if (/too\s+\w+[\w-]*\s+to\s+\w+/.test(lower))
        score += 18;
    if (/leave me alone|i'm\s+\w+|i am\s+\w+/.test(lower))
        score += 16;
    if (/\bzero\s+\w+/.test(lower))
        score += 14;
    if (/\bnot\s+\w+/.test(lower) || />/.test(slogan))
        score += 12;
    if (/\bunder\s+the\b|\boff\s+the\b|\bout\s+of\b/.test(lower))
        score += 10;
    if (/\bmy\s+game\b|\bhits different\b|\bbetter\b/.test(lower))
        score += 10;
    return clamp(score, 0, 30);
}
function computeFragmentPenalty(slogan) {
    const clauses = slogan.split(/[.:/]/).map((part) => part.trim()).filter(Boolean);
    let penalty = 0;
    if (clauses.length >= 2 && clauses.every((clause) => clause.split(/\s+/).length <= 2))
        penalty += 18;
    if (/^\w+\.\s*\w+\.?$/i.test(slogan))
        penalty += 20;
    if (/\bthe real too\b|\bstill too\b/i.test(slogan))
        penalty += 18;
    if (/^[A-Z\s.,!]+$/.test(slogan) && slogan.split(/\s+/).length > 3)
        penalty += 10;
    return clamp(penalty, 0, 30);
}
function heuristicPatternScore(slogan) {
    const clauses = slogan.split(/[|/:.-]/).map((part) => part.trim()).filter(Boolean);
    ...
    if (/[|/:-]/.test(slogan))
        score += 6;
    if (/^[A-Z0-9\s]+$/.test(slogan))
        score += 4;
    const uniqueWords = new Set(slogan.toLowerCase().split(/\s+/)).size;
    if (uniqueWords >= Math.max(2, wordCount - 1))
        score += 10;
    return clamp(score, 0, 100);
}
function computeConfidence(finalScore, wearability, memorability, genericPenalty, marketSignalScore) {
...
async function refineWithGPT4o(slogans, niche, audience) {
    if (slogans.length === 0)
        return [];
 ...
 */
async function generateBehavioralSlogans({ niche, behaviors, count = 20, }) {
    try {
...
`;
        ...
        try {
            ...
            if (Array.isArray(parsed.slogans))
                return parsed.slogans.map(String).slice(0, count);
        }
        catch (_) {
...
// ─── Core Scoring ─────────────────────────────────────────────────────────────
function scoreSlogan(slogan, salesSignals, mode, niche, batchStats = {}, clusters = []) {
...
function getPatternUsageStats(candidates) {
    const stats = {};
...
function isOverused(family, stats) {
    return (stats[family] || 0) > 0.25; // 25% threshold
}
function detectPattern(slogan) {
    const parts = slogan.split(/[|/:.-]/).map((part) => part.trim()).filter(Boolean);
    const words = slogan.split(/\s+/).length;
    if (parts.length >= 3)
        return "stacked_multi_clause";
    if (parts.length === 2 && words <= 6)
...
function inferPersona(niche, audience) {
    const lower = (niche + " " + (audience || "")).toLowerCase();
    for (const p of PERSONAS) {
        if (p.key === "broad_market")
            continue;
...
function buildReasons(scored) {
    const reasons = [];
    if (scored.confidence >= 80)
        reasons.push("High confidence for UI surfacing");
    if (scored.wearability >= 80)
    ...
    if (scored.punchScore >= 75)
        reasons.push("Punchy structure");
    if (scored.visualFit >= 75)
        reasons.push("Works well as visual text");
    if (scored.hookScore >= 70)
        reasons.push("Opens with a hook");
    if (scored.marketSignalScore >= 40)
        reasons.push("Backed by market signals");
    if (reasons.length === 0)
        reasons.push("Solid base candidate");
    return reasons;
}
function chooseBucket(finalScore, hookScore) {
    if (finalScore >= 72)
        return "topPicks";
    if (finalScore >= 56 || hookScore >= 68)
        return "boldPicks";
    return "experimental";
}
...
function diversifyRanked(ranked, niche = "") {
    const usedSignatures = new Set();
...
function remixSlogans(seed, identities, emotions) {
    const results = [];
...
function buildCollections(ranked) {
    ...
    for (const r of ranked) {
        ...
        if (assigned.has(key))
            continue;
        assigned.add(key);
        if (r.bucket === "topPicks")
            topPicks.push(r);
        else if (r.bucket === "boldPicks")
            boldPicks.push(r);
        else
            experimental.push(r);
    }
    ...
    for (const pat of patterns) {
        ...
        if (hasPlaceholder && anchors.length > 0) {
            ...
            for (const e of emotions.slice(0, 3))
                variants.push(template.replace(/\[EMOTION\]/gi, e));
        }
        else {
            ...
            if (anchors.length > 0) {
                ...
                if (anchors[1])
                    variants.push(`${anchors[1]} ${candidate}`);
            }
            else {
        ...
        for (const v of variants) {
            ...
            if (!isGeneric(cleaned) && cleaned.split(/\s+/).length <= 8)
                results.push(cleaned);
        }
    }
    return dedupeStrings(results).slice(0, 48);
}
function selectTopPerformers(ranked) {
    const cutoff = Math.max(4, Math.ceil(ranked.length * 0.1));
    return ranked.slice(0, cutoff);
}
// ─── AI Slogan Generator ──────────────────────────────────────────────────────
async function generateAISlogans(niche, audience, personaLabel, mode, dominantClusters = []) {
    // Market intelligence
    ...
    try {
        ...
        const filtered = unique.filter((s) => {
            ...
            if (lower.includes("too ") && lower.includes(" to "))
                return false;
            if (lower.startsWith("built for"))
                return false;
            if (lower.includes("no apologies"))
                return false;
            if (lower.includes("state of mind"))
                return false;
            return true;
        });
 ...
 */
async function expandPatternFamilies(niche, count = 5, behaviors = []) {
    try {
        const response = await (0, aiGateway_1.chatCompletionSafe)({
            model: "gpt-4o-mini",
            temperature: 0.88,
        ...
        try {
            ...
            if (Array.isArray(parsed.patterns))
                return parsed.patterns.map(String).slice(0, count);
        }
        catch (_) {
 ...
 */
async function mutatePatternFamily(basePattern, niche, count = 8, behaviors = []) {
    if (!basePattern || basePattern.trim().length === 0)
        return [];
    try {
...
function buildEliteSloganEngine(input) {
...
function resolveExecMode(context, requested) {
    ...
    if (ctx === "dashboard")
        return "balanced";
    if (ctx === "design_studio" || ctx === "autopilot")
        return "elite";
    return "balanced";
}
 ...
 */
async function generateHighPotentialSlogans(input) {
    ...
    try {
        const cached = await cache_1.globalCache.getAsync(cacheKey);
    ...
    try {
        cache_1.globalCache.set(cacheKey, normalized, input.cacheTtlSec ? input.cacheTtlSec * 1000 : ttlMap[execMode]);
    }
    catch {
...
function extractEmotions(niche, audience) {
    const persona = inferPersona(niche, audience);
    return buildNicheLexicon({ niche, audience }, persona, "safe").emotionWords;
}
function generateVariants(slogan) {
    const variants = [
        slogan,
        slogan.toUpperCase(),
        slogan.replace(/\.$/, "!"),
...
function buildPromptDescription(style, niche) {
...
function extractPromptDescription(prompt, slogan, style, niche) {
...
function buildStructuredImagePrompt(slogan, description, style) {
...
async function learnPattern(pattern, slogan, niche) {
    try {
...
async function getPatternScore(nicheOrPattern, maybePattern) {
    try {
        ...
        const row = await prisma_1.prisma.sloganPattern.findUnique({
...
async function predictVirality(slogan, niche, tags) {
...
async function getTrendScore(niche, tags) {
    try {
        const nicheKey = niche.trim().toLowerCase();
        const recent = await prisma_1.prisma.marketSignal.findFirst({
            where: { niche: nicheKey },
            orderBy: { observedAt: "desc" },
        });
        if (recent)
            return clamp(Math.round(recent.score * 100), 0, 100);
    }
    catch (_) { /* fallback */ }
    return clamp(50 + (tags || []).length * 3, 0, 100);
}
async function syncMarketplace(niche, slogan) {
    const nicheKey = niche.trim().toLowerCase();
    const score = clamp(Math.random() * 0.4 + 0.55, 0, 1);
    try {
        await prisma_1.prisma.marketSignal.create({
            data: {
                niche: nicheKey,
                text: slogan || niche,
