...
exports.createTrendSnapshot = createTrendSnapshot;
exports.generateMarketSignals = generateMarketSignals;
...
function createTrendSnapshot(seed = null, fallbackScore = 65) {
    ...
    const growthCandidate = firstFiniteNumber(seed?.trend?.signals?.growthAcceleration, seed?.growthAcceleration, typeof seed?.profitScore === "number" && typeof seed?.viralScore === "number"
...
function generateBaseSignals(niche, trendSigs = null) {
...
function refineWithAI(base, aiSignals, trendSigs = null) {
...
function scoreWithMarketIntel(intel, market, trendSigs = null) {
    ...
    const opportunityScore = trendScore * 0.5 +
        demandScore * 0.3 +
    ...
    if (finalScore < 50)
        decision = 'SKIP';
    if (intel?.safe === false && decision === 'PUBLISH')
        decision = 'TEST';
    return {
