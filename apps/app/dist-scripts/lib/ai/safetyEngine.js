...
exports.runSafetyEngine = runSafetyEngine;
// ─── Entity Registry ──────────────────────────────────────────────────────────
...
function calculateRiskScore(entities) {
    if (entities.length === 0)
        return 0;
    const total = entities.reduce((sum, e) => sum + RISK_WEIGHTS[e.type], 0);
    return Math.min(total, 100);
}
function sanitize(niche, entities) {
    let result = niche;
    for (const entity of entities) {
        // find the original pattern to replace accurately
        const ep = ENTITY_PATTERNS.find((p) => p.type === entity.type && p.replacement === entity.replacement);
        if (ep) {
...
// ─── Public API ───────────────────────────────────────────────────────────────
function runSafetyEngine(niche) {
