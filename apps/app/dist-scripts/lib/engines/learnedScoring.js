...
exports.getLearnedBoost = getLearnedBoost;
 ...
 */
async function getLearnedBoost(slogan, niche) {
    const pattern = (0, patternMemory_1.extractPattern)(slogan);
    const nicheKey = (niche || "global").trim().toLowerCase().slice(0, 60);
    try {
        const patterns = await (0, patternMemory_1.getTopPatterns)(nicheKey, 50);
 ...
 */
async function buildBoostMap(niche) {
    const patterns = await (0, patternMemory_1.getTopPatterns)(niche, 50);
    const map = new Map();
 ...
 */
function baseHeuristicScore(slogan) {
    let score = 0;
    ...
    if (/[&+]/.test(slogan))
        score += 4;
    // Emotion triggers
    if (/(obsessed|addicted|passionate|devotee|proud|living for|always|never|still)/i.test(slogan))
        score += 10;
    // Identity structure
    if (/\b(person|parent|lover|fan|maker|creator|runner|rider|reader)\b/i.test(slogan))
        score += 8;
    // Contrast / tension (high converting structure)
    if (/ over | not | vs | > /i.test(slogan))
        score += 12;
    return score;
}
/**
 ...
 */
async function rankSlogans(slogans, niche) {
    ...
    const scored = slogans.map((text) => {
        const pattern = (0, patternMemory_1.extractPattern)(text);
 ...
 */
function selectWinners(ranked, percentage = 0.1) {
    if (ranked.length === 0)
        return ranked;
 ...
 */
async function getWinningSlogans(slogans, niche, percentage = 0.1) {
    if (slogans.length === 0)
        return [];
    try {
        ...
        return slogans.map((text) => ({
            ...
            score: 0,
            learnedBoost: 0,
            pattern: (0, patternMemory_1.extractPattern)(text),
            hasEvidenced: false,
        }));
    }
}
