...
let usageMetricDegradedReason = null;
let usageMetricVerificationInFlight = null;
const USAGE_BACKOFF_BASE_MS = 1000;
const USAGE_BACKOFF_MAX_MS = 30000;
function isMissingUsageMetricTableError(err) {
...
async function usageMetricTableExists() {
    ...
    usageMetricVerificationInFlight = prisma_1.prisma.usageMetric
        .count()
...
function markUsageMetricUnavailable(reasonOverride) {
...
function markUsageMetricAvailable() {
...
function shouldSkipUsageMetricAccess() {
...
function zeroUsage(userId, status = "healthy", degradedReason) {
    return {
        userId,
        totalTokens24h: 0,
        totalTokens30d: 0,
...
function parseFeatureFromType(type) {
    if (!type.startsWith("ai:"))
        return null;
    const parts = type.split(":");
    return parts.length >= 3 ? parts[1] : null;
}
async function handleUsageMetricFailure(err) {
...
const PLAN_LIMITS = {
    free: {
        ...
        perFeature24h: {
            "strategy.single": 25000,
            "factory.bulkChunk": 30000,
            "factory.autopilotChunk": 35000,
            "discovery.generateNiches": 20000,
            "discovery.enrichNiches": 20000,
        },
    },
    pro: {
        totalTokens24h: 1000000,
    },
    pro_yearly: {
        totalTokens24h: 1000000,
    },
...
async function recordAIUsage(record) {
    ...
    if (totalTokens <= 0 || shouldSkipUsageMetricAccess())
        return;
    const userId = record.userId || "system";
    const type = `ai:${record.feature}:${record.model}:tokens`;
    try {
        await prisma_1.prisma.usageMetric.create({
            data: {
                userId,
                type,
                value: totalTokens,
...
async function getAIUsageSummaryForUser(userId) {
    const now = new Date();
    ...
    try {
        const metrics = await prisma_1.prisma.usageMetric.findMany({
            where: {
                userId,
...
async function getUserPlanAndLimits(userId) {
    const plan = await getUserPlan(userId);
    const limits = PLAN_LIMITS[plan] || PLAN_LIMITS.free;
    return { plan, limits };
}
async function ensureUsageAllowed(userId, feature) {
    try {
