    ...
    for (const pat of patterns) {
        if (pat.includes('[ANCHOR]')) {
            for (const a of anchors)
                results.push(pat.replace(/\[ANCHOR\]/gi, a));
        }
        else {
            results.push(pat);
            if (anchors[0])
                results.push(`${anchors[0]} ${pat}`);
        }
    }
...
function scoreSlogan(s) {
    ...
    if (/[?]/.test(s))
        sc += 18;
    if (/dink|serve|smash|play|got|insiders|kitchen|sorry|just one/.test(lower))
        sc += 14;
    ...
    if (wc <= 5)
        sc += 6;
    if (/energy|real|certified|insiders/.test(lower))
        sc += 8;
    if (/vibe|mood|legend|whisperer/.test(lower))
        sc -= 20;
...
function refineMock(s) {
    ...
    if (/play/i.test(s))
        return s.replace(/Play/i, 'Just One More Game');
    if (/got/i.test(s))
        return s.replace(/Got/i, "Got 'em");
    if (/mode/i.test(s))
        return s.replace(/Mode/i, 'Mode — Dink Edition');
    if (/energy/i.test(s))
        return s.replace(/Energy/i, 'Energy');
    if (/insiders/i.test(s))
        return s.replace(/Insiders/i, 'Insiders — No Volley Zone');
    if (/kitchen/i.test(s))
        return s.replace(/Kitchen/i, 'Kitchen Rules');
    if (/dink/i.test(s) && /responsibly/i.test(s))
        return s;
    // fallback shortener
    return s;
}
