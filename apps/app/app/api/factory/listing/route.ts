...
const RepackageSchema = z.object({
  ...
  rankedSearchTerms: z.array(z.object({
    term: z.string().trim().min(1).max(160),
    relevance: z.number().min(0).max(1),
    confidence: z.number().min(0).max(1),
    naturalLanguageFit: z.number().min(0).max(1),
    evidenceSupport: z.number().min(0).max(1),
    source: z.enum(["market", "profile", "derived"]),
    usage: z.enum(["BACKEND_ONLY", "CUSTOMER_COPY", "BOTH"]),
  })).max(40).optional(),
