...
// ---------------------------------------------------------------------------
// TrendForge Compliance Engine
// Covers: Merch by Amazon · Etsy · Redbubble
//
// Rule taxonomy:
//   TRADEMARK   – registered brand names, product names, slogans
//   COPYRIGHT   – character names, song titles/lyrics, movie/show quotes
//   CELEBRITY   – real person name or likeness without consent
//   HATE        – hate speech, slurs, symbols, extremist orgs
//   VIOLENCE    – graphic violence, weapons glorification
//   ADULT       – sexual content, nudity references
//   FALSE_CLAIM – medical/health claims, FDA/certification references
//   PLATFORM    – platform-specific listing rules (Amazon only policy violations)
//   DESIGN_QUALITY – content that leads to poor customer experience
// ---------------------------------------------------------------------------
...
export type Platform = "amazon" | "etsy" | "redbubble" | "all";

export interface ComplianceViolation {
    category: ViolationCategory;
    term: string;
    reason: string;
    /** Which platforms flag this exact violation */
    platforms: Platform[];
    /** Whether the entire product should be rejected (vs. just flagged) */
    blocks: boolean;
}
...
function fuzzyMatch(text: string, term: string): boolean {
    if (term.length < 4) return false; // Avoid false positives on short words
...
function isDerivative(text: string): boolean {
...
const TRADEMARK_RULES: Rule[] = [
    ...
    ...[
        ...
        pattern: phrase(t),
        category: "TRADEMARK" as ViolationCategory,
...
const COPYRIGHT_RULES: Rule[] = [
    ...
    ...[
        ...
        pattern: phrase(t),
        category: "COPYRIGHT" as ViolationCategory,
...
const CELEBRITY_RULES: Rule[] = [
    ...[
        ...
        pattern: phrase(t),
        category: "CELEBRITY" as ViolationCategory,
...
const DESIGN_QUALITY_RULES: Rule[] = [
    {
...
function scanText(text: string): ComplianceViolation[] {
...
function computeRiskLevel(violations: ComplianceViolation[]): RiskLevel {
    if (violations.length === 0) return "CLEAR";
...
function platformViolations(
    violations: ComplianceViolation[],
    platform: "amazon" | "etsy" | "redbubble",
): ComplianceViolation[] {
    return violations.filter(
        (v) => v.platforms.includes("all") || v.platforms.includes(platform),
    );
}
...
const CATEGORY_SUGGESTIONS: Record<ViolationCategory, string> = {
    TRADEMARK:
        "Remove or rephrase any brand names. Use generic descriptors instead (e.g., 'athletic wear' instead of a brand name, 'coffee shop vibes' instead of a cafe brand).",
    COPYRIGHT:
        "Replace copyrighted character/franchise references with original archetypes (e.g., 'space warrior' instead of a Star Wars character, 'wizard academy' instead of Hogwarts).",
    CELEBRITY:
        "Remove the celebrity name entirely. Reference their *aesthetic* or profession generically (e.g., 'pop star energy', 'basketball legend vibes') without naming the person.",
    ...
    VIOLENCE:
        "Rephrase all violent language. Passive references to strength or resilience are fine — direct glorification of harm is not.",
    ADULT:
        "Remove all adult/explicit content. Keep designs and copy family-friendly for all age groups.",
    FALSE_CLAIM:
        "Remove health claims, charity promises, and self-awarded rankings. Only factual, verifiable statements are allowed.",
    PLATFORM:
        "Remove shipping/delivery references, review solicitations, and off-platform links from listing copy.",
    DESIGN_QUALITY:
        "Replace placeholder text with real design content. Amazon reviewers reject listings that appear unfinished.",
};

function buildSuggestions(violations: ComplianceViolation[]): string[] {
...
export interface SafetyResult {
    safe: boolean;
    reason?: "trademark_exact" | "trademark_fuzzy" | "derivative_content";
    riskLevel: RiskLevel;
    violations: ComplianceViolation[];
    platforms: ComplianceReport["platforms"];
    suggestions: string[];
}
 ...
 */
export function checkCompliance(product: ProductFields): ComplianceReport & { safetyReason?: string } {
    const fields = [
        product.niche ?? "",
        product.slogan ?? "",
    ...
    if (blocks.length > 0) safetyReason = "trademark_exact";
 ...
 */
export function checkSlogan(slogan: string): ComplianceReport {
 ...
 */
export function checkText(text: string): ComplianceReport {
 ...
 */
export function filterSafeSlogans(
    slogans: string[],
): { safe: string[]; removed: Array<{ slogan: string; reason: string }> } {
    const safe: string[] = [];
    const removed: Array<{ slogan: string; reason: string }> = [];
