...
export type RecordMerchOutcomeFeedbackInput = {
    userId: string;
    niche: string;
    platform?: string;
    slogan: string;
...
function toObservedAt(value?: string | Date): Date {
...
function inferPattern(slogan: string): string | undefined {
    const lower = slogan.toLowerCase();
    if (lower.includes(">")) return "contrast";
    if (/plot twist/.test(lower)) return "plot_twist";
    if (/not .* just /.test(lower)) return "not_x_just_y";
    if (/still .* still /.test(lower)) return "still";
    if (/^built for /.test(lower) || / built me$/.test(lower)) return "built_for";
    if (/^i am /.test(lower)) return "identity";
    if (/\. no apologies\.$|\. full volume\.$/.test(lower)) return "declaration";
    return undefined;
}
...
function bucketToSignal(bucket?: AggregateBucket): SignalLike | undefined {
    ...
    if (evidenceMagnitude <= 0) return undefined;
...
function blendSignals(layers: Array<{ signal?: SignalLike; baseWeight: number }>): SignalLike | undefined {
    let ctrTotal = 0;
    let ctrWeight = 0;
...
function normalizeExistingSignals(input: unknown): Map<string, { slogan: string; signal: SignalLike }> {
    const signals = new Map<string, { slogan: string; signal: SignalLike }>();

    if (Array.isArray(input)) {
        for (const entry of input) {
            if (!entry || typeof entry !== "object") continue;
            const candidate = entry as { slogan?: unknown } & SignalLike;
            if (typeof candidate.slogan !== "string" || !candidate.slogan.trim()) continue;
    ...
    for (const [key, value] of Object.entries(input as Record<string, unknown>)) {
        ...
        const candidate = value as SignalLike;
 ...
 */
async function updatePatternScoreFromFeedback(
    niche: string,
    slogan: string,
    pattern: string | null,
    impressions: number,
    ...
    if (impressions < 20 && clicks < 5) return;
    ...
    if (impressions > 0) {
        ...
        if (ctr >= 0.05) performanceFactor += 0.35;
        else if (ctr >= 0.03) performanceFactor += 0.15;
        else if (ctr < 0.01) performanceFactor -= 0.30;
        else if (ctr < 0.02) performanceFactor -= 0.15;
    }
    if (clicks > 0) {
        const convRate = orders / clicks;
        if (convRate >= 0.09) performanceFactor += 0.20;
        else if (convRate >= 0.06) performanceFactor += 0.10;
        else if (convRate < 0.02) performanceFactor -= 0.15;
    }
    performanceFactor = Math.max(0.3, Math.min(2.5, performanceFactor));

    try {
        const existing = await prisma.sloganPattern.findUnique({
        ...
        if (existing) {
            ...
            await prisma.sloganPattern.update({
                where: { niche_pattern: { niche: nicheKey, pattern: patternKey } },
                data: { score: newScore, uses: { increment: 1 } },
            });
        } else {
            await prisma.sloganPattern.create({
                data: {
                    niche: nicheKey,
                    pattern: patternKey,
...
export async function recordMerchOutcomeFeedback(input: RecordMerchOutcomeFeedbackInput) {
    ...
    try {
        const record = await prisma.merchOutcomeFeedback.create({
            data: {
                userId: input.userId,
                niche,
        ...
        updatePatternScoreFromFeedback(
            niche,
            slogan,
            input.pattern?.trim() || null,
            sanitizeCount(input.impressions),
            sanitizeCount(input.clicks),
            sanitizeCount(input.orders),
        ).catch(() => { /* non-blocking */ });
...
export async function getPersistedSalesSignalsForRankedSlogans(params: {
    userId?: string;
    niche: string;
    platform?: string;
    rankedSlogans: RankedSloganLike[];
}): Promise<PersistedSignalMap> {
    ...
    const cutoff = new Date(Date.now() - FEEDBACK_LOOKBACK_DAYS * 86_400_000);
    ...
    try {
        ...
        const patternBuckets = new Map<string, AggregateBucket>();
        ...
        const results: PersistedSignalMap = {};
...
export function mergeSalesSignalsInputs(existing: unknown, learned: PersistedSignalMap): PersistedSignalMap | unknown {
    const existingSignals = normalizeExistingSignals(existing);
    const learnedEntries = Object.entries(learned);

    if (existingSignals.size === 0 && learnedEntries.length === 0) return existing;
    if (existingSignals.size === 0) return learned;
    if (learnedEntries.length === 0) return existing;

    const merged = new Map<string, { slogan: string; signal: SignalLike }>();
    ...
    return Object.fromEntries(
        [...merged.values()].map((entry) => [entry.slogan, entry.signal])
    );
}