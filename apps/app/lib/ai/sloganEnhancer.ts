...
export type NicheCategory = keyof typeof NICHE_CATEGORY_KEYWORDS;

export interface EnhancedSloganSet {
  slogans: string[];
 ...
 */
export function detectNicheCategories(niche: string): NicheCategory[] {
  const lower = niche.toLowerCase();
  const scored = Object.entries(NICHE_CATEGORY_KEYWORDS)
    .map(([cat, words]) => ({
      cat: cat as NicheCategory,
 ...
 */
export function getCategoryVocab(categories: NicheCategory[]): string[] {
  const vocab: string[] = [];
 ...
 */
const SLOGAN_BANNED_TERMS: string[] = [
  // True crime IP
 ...
 */
export function isSafeSlogan(slogan: string): boolean {
  const lower = slogan.toLowerCase().trim();

  // Trademark / IP check
  if (SLOGAN_BANNED_TERMS.some((term) => lower.includes(term))) return false;

  // Trademark symbols (explicit) — treat as unsafe
  if (/\u2122|\u00AE|\btm\b|\br\b/i.test(slogan)) return false;
  ...
  if (scaffoldMatch) {
    ...
    if (SCAFFOLD_MODIFIER_ADJECTIVES.has(firstWord)) return false;
  }
  ...
  if (lower.includes(" > ")) {
    ...
    if (SCAFFOLD_MODIFIER_ADJECTIVES.has(lhsFirst)) return false;
  }
 ...
 */
export function isWearable(s: string): boolean {
  ...
  if (/^i am|^this is/i.test(lower)) return false;
 ...
 */
export function transformStatement(s: string): string {
 ...
 */
function tidySlogan(s: string): string {
  if (!s) return s;
  let t = s.trim();
 ...
 */
export function makeConversational(s: string): string {
  return s
 ...
 */
export function scoreCrossNicheAlignment(slogan: string, categories: NicheCategory[]): number {
  if (categories.length === 0) return 0;
  const lower = slogan.toLowerCase();
  const coveredCount = categories.filter((cat) =>
    (NICHE_CATEGORY_KEYWORDS[cat] ?? []).some((w) => lower.includes(w)),
  ).length;

  if (coveredCount >= 2) return 20;
 ...
 */
export function coversAtLeastOneCategory(slogan: string, categories: NicheCategory[]): boolean {
 ...
 *   4. Deduplicate
 */
export function filterAndEnhanceSlogans(
  slogans: string[],
  niche: string,
): EnhancedSloganSet {
  ...
  const sorted = [...enhanced].sort(
    (a, b) => scoreCrossNicheAlignment(b, categories) - scoreCrossNicheAlignment(a, categories),
  );
