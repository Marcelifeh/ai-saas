 ...
 */
export function extractPattern(slogan: string): string {
    const lower = slogan.toLowerCase().trim();

    if (/\bvs\.?\b/.test(lower) || / > /.test(lower)) return "comparison";
    if (lower.includes(" not ") && lower.includes(" just ")) return "not_x_just_y";
    if (/plot twist/.test(lower)) return "plot_twist";
    if (/^still\b/.test(lower)) return "still";
    if (/^built for\b/.test(lower) || /\bbuilt me$/.test(lower)) return "built_for";
    if (/^(i am|i'm)\b/.test(lower)) return "identity_declaration";
    if (/\. no apologies\.$|\. full volume\.$/.test(lower)) return "declaration";
    if (/^(not your|not a typical|not just a)\b/.test(lower)) return "not_your";
    if (lower.includes(" over ")) return "contrast_over";
    if (/^(fueled by|powered by|running on)\b/.test(lower)) return "fueled_by";
    if (/^\w+\. \w+\. \w+\.?$/.test(lower)) return "triple_noun";
    if (/^(zero|no) [a-z]+/.test(lower)) return "zero_noun";
    if (/\bmode\b/.test(lower)) return "mode";
    if (/\bvibes?\b/.test(lower)) return "vibes";
    if (/[?]$/.test(lower)) return "rhetorical_question";
    if (lower.split(/\s+/).length <= 3) return "short_punch";
...
export interface SloganPerformanceInput {
    slogan: string;
    niche: string;
    impressions?: number;
    clicks?: number;
 ...
 */
export async function recordSloganPerformance(input: SloganPerformanceInput): Promise<void> {
    ...
    try {
        ...
        if (totalClk > 0) {
            if (conversion >= 0.09) performanceFactor += 0.20;
            else if (conversion >= 0.06) performanceFactor += 0.10;
            else if (conversion < 0.02) performanceFactor -= 0.15;
        }
 ...
 */
export async function getTopPatterns(niche: string, limit = 20): Promise<PatternRecord[]> {
    const nicheKey = niche.trim().toLowerCase().slice(0, 60);
    try {
        return (await prisma.sloganPattern.findMany({
 ...
 */
export async function getGlobalTopPatterns(limit = 10): Promise<PatternRecord[]> {
    try {
        return (await prisma.sloganPattern.findMany({
            where: { uses: { gte: 3 } },
